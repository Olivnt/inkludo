# fiken_app.py — Voice Agent (stable) + Auto-launch relay + Fiken picker + RAG Text Chat + Health + Optional Weaviate tools
import os
import sys
import time
import json
...
st.title("🎙️ Inkludo Voice Agent")
st.caption("Hands-free voice with on-screen transcript. Tool-calls hit your invoice index before answering.")
...
def _start_relay_subprocess() -> subprocess.Popen:
    ...
    env = os.environ.copy()
    cmd = [sys.executable, "-m", "uvicorn", RELAY_MODULE, "--host", "0.0.0.0", "--port", str(RELAY_PORT)]
    ...
    if show_sidebar:
        with st.sidebar.expander("Relay (local)"):
            ...
            if col1.button("Check health"):
                st.success("Healthy") if _relay_health_ok() else st.error("Not responding")
...
def explain_http_error(e: requests.HTTPError) -> str:
    ...
    if resp.status_code == 403:
        return ("🔒 403 Forbidden — enable *Foretak → Tilleggstjenester → API/Netshop* or use a Test-company. "
                f"Details: {j}")
    if resp.status_code == 401:
        return "🔑 401 Unauthorized — token expired/invalid."
    return f"HTTP {resp.status_code}: {j}"

# ----------------------------- Weaviate helpers (optional) -----------------------------
...
def weaviate_count_objects(company_slug: str | None = None) -> int:
    client = connect_weaviate_v3()
    if company_slug:
        # Weaviate v3 raw GraphQL doesn't accept variables easily; use 'where' through .aggregate if preferred.
        q = (
            '{ Aggregate { InvoiceChunk(where: {path:["company_slug"], operator: Equal, valueString: "'
            + company_slug.replace('"', '\\"')
            + '"}) { meta { count } } } }'
        )
        res = client.query.raw(q)
    else:
...
CONDENSE_PROMPT = ChatPromptTemplate.from_template(
    """Rewrite the user question into a focused search query for invoices.
Keep supplier names, dates, amounts, currencies, and invoice numbers.
...
ANSWER_PROMPT = ChatPromptTemplate.from_template(
    """You are an assistant answering questions about invoices.
Use ONLY the information in <context>. If it's missing, say you cannot find it.
Always end with: "Sources: [numbers]".
...
def _summarize_history(chat_history):
    if not chat_history:
        return "N/A"
    pairs = chat_history[-6:]
    return " | ".join([f"U: {u} A: {a}" for (u, a) in pairs])

def rag_chat_answer(user_q: str, chat_history: list, company_slug: str | None, top_k: int = 6):
    # Quick count path
    q_lower = (user_q or "").lower()
    if ("how many" in q_lower or "hvor mange" in q_lower) and _HAS_WEAVIATE:
        try:
            cnt = weaviate_count_objects(company_slug=company_slug)
            org = f" for **{company_slug}**" if company_slug else ""
            return f"Du har **{cnt}** indekserte fakturaer{org}.  \nSources: [1]", []
        except Exception:
            pass

    if not (_HAS_WEAVIATE and _HAS_EMB):
        return "Jeg har ikke tilgang til indekset akkurat nå.", []
    ...
    condensed = CONDENSE_PROMPT | llm
    q2 = condensed.invoke({"history": _summarize_history(chat_history), "question": user_q}).content.strip()
    st.info(f"🔎 Rewritten search query: **{q2}**")

    client = connect_weaviate_v3()
    emb = _embedder()
    vs = LCWeaviate(client=client, index_name="InvoiceChunk", text_key="text", by_text=False, embedding=emb)

    search_kwargs = {"k": max(8, top_k)}
    if company_slug:
        search_kwargs["where"] = {"path": ["company_slug"], "operator": "Equal", "valueString": company_slug}

    docs = vs.similarity_search(q2, **search_kwargs)
    st.caption(f"Retriever hits (similarity): {len(docs)}")
    if not docs:
        try:
            where = search_kwargs.get("where")
            retriever = vs.as_retriever(search_type="mmr", search_kwargs={"k": 12, "fetch_k": 30, **({"where": where} if where else {})})
            docs = retriever.get_relevant_documents(q2)
            st.caption(f"Retriever hits (MMR): {len(docs)}")
        except Exception as e:
            st.caption(f"MMR retrieval failed: {e}")

    if not docs:
        return ("Jeg finner ikke dette i de indekserte fakturaene. "
                "Synkroniser først, og spør med leverandør/dato/beløp."), []

    numbered = []
    for i, d in enumerate(docs, start=1):
        m = d.metadata or {}
        meta = f"Invoice {m.get('invoice_no') or 'N/A'} | Date {m.get('issue_date') or 'N/A'} | Supplier {m.get('supplier') or 'N/A'} | Total {m.get('amount_total') or 'N/A'} {m.get('currency') or ''}"
        numbered.append(f"[{i}] {meta}\n{d.page_content}")

    context_text = "\n\n".join(numbered)
    chain = ANSWER_PROMPT | llm
    ans = chain.invoke({"context": context_text, "question": user_q}).content
    return ans, docs
...
# ----------------------------- Voice Agent UI (with stability fixes) -----------------------------
...
<script>
  ...
  let awaitingToolDecision = false;  // NEW: suppress babble before tool decision
      ...
      dc.onopen = () => {{
        const sessionUpdate = {{
          ...
          "session": {{
            "tools": toolSchemas,
            "voice": "{voice}",
            "temperature": 0.15,  // NEW: lower randomness
            "instructions":
              "If the user asks about invoices, ALWAYS call invoice_search first and wait for results. " +
              "Do not speak interim thoughts. Never say goodbye or end on your own. " +
              "After the tool returns, answer briefly in Norwegian and cite key fields.",
            "turn_detection": {{
              "type": "server_vad",
              "threshold": 0.6,           // NEW: calmer VAD
              "prefix_padding_ms": 250,
              "silence_duration_ms": 1100 // NEW: longer turn end
            }},
      ...
      dc.onmessage = async (e) => {{
        try {{
          ...
          if (msg.type === "response.created") {{
            // A new response cycle started — hold deltas until we know if a tool will be called
            awaitingToolDecision = true;
          }}
          ...
          if (msg.type === "response.function_call.created" && msg.call_id) {{
            ...
            awaitingToolDecision = false;

            // NEW: stop any current speech/typing until tool output arrives
            try {{ dc.send(JSON.stringify({{ type: "response.cancel" }})); }} catch (_) {{}}
            currentTextNode = null;
          }}

          if (msg.type === "response.output_text.delta") {{
            // NEW: suppress model babble before tool decision is known
            if (awaitingToolDecision) return;
          ...
          if (msg.type === "response.function_call_arguments.done") {{
            ...
            if (name === "invoice_search") {{
              ...
              // NEW: only speak after tool output; keep it short and factual
              dc.send(JSON.stringify({{
                "type": "response.create",
                "response": {{
                  "instructions": "Using ONLY the tool output just provided, answer briefly in Norwegian. " +
                                   "If there are no results, say that clearly. No chit-chat, no goodbye."
                }}
              }}));
            }}
          }}
...
if not _HAS_WEAVIATE or not _HAS_EMB or need_envs:
    ...
    if not fiken_token or not company_slug:
            ...
            with c1:
                if st.button("Sync last 3 years → Index"):
                    try:
                        ...
                        before = weaviate_count_objects(company_slug=company_slug)
                        st.write(f"Vector DB count (before for {company_slug}): **{before}**")
                        n = batch_upsert_invoices(invs, company_slug)
                        after = weaviate_count_objects(company_slug=company_slug)
        ...
        with st.expander("🧪 Vector DB status"):
            try:
                cnt_all = weaviate_count_objects()
                st.write(f"Objects in `InvoiceChunk` (all): **{cnt_all}**")
                cnt_company = weaviate_count_objects(company_slug=company_slug)
                st.write(f"Objects for **{company_slug}**: **{cnt_company}**")
...
user_q = st.chat_input("Ask about your invoices… (tip: include supplier/date/amount)")
...
if user_q:
    ...
    with st.chat_message("assistant"):
        answer = ""
        docs = []
        use_rag = _HAS_WEAVIATE and _HAS_EMB and os.getenv("WEAVIATE_URL") and os.getenv("WEAVIATE_API_KEY")
        try:
            if use_rag:
                with st.spinner("Søker i indekset…"):
                    answer, docs = rag_chat_answer(user_q, st.session_state.chat, company_slug, top_k=6)
            else:
                client = OpenAI()
        ...
        if docs:
            with st.expander("Sources"):
                for i, d in enumerate(docs, start=1):
                    md = d.metadata or {}
                    line = (
                        f"**[{i}]** Invoice `{md.get('invoice_no') or 'N/A'}` • "
                        f"Date: {md.get('issue_date') or 'N/A'}` • "
                        f"Supplier: {md.get('supplier') or 'N/A'}` • "
                        f"Total: {md.get('amount_total') or 'N/A'} {md.get('currency') or ''}"
                    )
                    st.markdown(line)
                    if md.get("pdf_url"):
                        st.markdown(f"[📄 View PDF]({md.get('pdf_url')})")
